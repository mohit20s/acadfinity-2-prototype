...
export default function SeekhoLessonPlayerPage() {
  ...
  // State for Materials Drawer
  const [isMaterialsOpen, setIsMaterialsOpen] = useState(false);
  
  // NEW: State and Ref for Fullscreen Video
    ...
    if (!document.fullscreenElement) {
      try {
        if (videoContainerRef.current) {
          ...
          setIsMaterialsOpen(false); // Auto-close drawer if going fullscreen
        }
      } catch (err) {
  ...
  // Listen for ESC key or back gesture to update state if they exit fullscreen natively
  ...
  return (
    ...
    <div className="bg-black h-[100dvh] w-full text-white md:bg-slate-900 animate-in fade-in duration-500 overflow-hidden flex flex-col relative">
      ...
      <div className="w-full max-w-md mx-auto h-full bg-black md:shadow-2xl md:border-x border-slate-800 relative overflow-hidden flex flex-col">
        ...
        {/* =========================================
            1. THE FULL-SCREEN VIDEO PLAYER (Now with Fullscreen Ref)
            ========================================= */}
        <div 
          ref={videoContainerRef}
          className={cn(
            "relative w-full transition-all duration-500 ease-in-out bg-slate-900 z-10 flex flex-col",
            // If fullscreen, take 100vh. If drawer is open, shrink to 35vh. Otherwise, 100vh full screen!
            isFullscreen ? "h-[100dvh]" : isMaterialsOpen ? "h-[35vh]" : "h-[100dvh]" 
          )}
        >
          ...
          <div className="absolute top-0 left-0 right-0 p-3 pt-safe-area-inset-top z-20 flex justify-between items-start bg-gradient-to-b from-black/80 to-transparent">
            {/* If fullscreen, the back button exits fullscreen. Otherwise it goes back to LMS */}
            <button 
              onClick={isFullscreen ? toggleFullscreen : () => router.back()} 
              className="p-2 bg-black/40 backdrop-blur-md rounded-full hover:bg-white/20 transition-colors"
            >
            ...
            <button onClick={toggleFullscreen} className="p-2 bg-black/40 backdrop-blur-md rounded-full hover:bg-white/20 transition-colors">
          ...
          {/* Right Side Floating Actions (Hide when Drawer is Open or in Fullscreen landscape) */}
          <div className={cn(
          ...
          <div className={cn(
            ...
            {/* BUTTON TO OPEN MATERIALS DRAWER (Hide if Fullscreen is active) */}
          ...
          <div className="flex-1 overflow-y-auto pb-20 px-1">
            ...
            <div className="p-4 border-b border-white/5">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-emerald-500/30">
                  <div className="flex items-center gap-3"><div className="p-2 bg-emerald-500/20 text-emerald-400 rounded-lg"><FileText className="h-4 w-4" /></div><div><p className="text-xs font-bold text-emerald-400">Pre-Read: Core Concepts</p><p className="text-[9px] text-slate-400 uppercase font-bold mt-0.5">Completed • 5 mins</p></div></div>
                ...
                <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 opacity-50">
                  <div className="flex items-center gap-3"><div className="p-2 bg-slate-500/20 text-slate-400 rounded-lg"><FileText className="h-4 w-4" /></div><div><p className="text-xs font-bold text-white">Post-Read: Summary</p><p className="text-[9px] text-slate-400 uppercase font-bold mt-0.5">Unlock by finishing video</p></div></div>
            ...
            <div className="p-4 border-b border-white/5">
              ...
              <div className="p-5 bg-gradient-to-br from-purple-500/20 to-blue-500/10 border border-purple-500/30 rounded-2xl relative overflow-hidden">
                 ...
                 <div className="relative z-10"><span className="text-[9px] font-black uppercase tracking-widest bg-purple-500 text-white px-2 py-0.5 rounded">Assignment</span><h4 className="font-bold text-white text-base mt-2 mb-1">Variables Worksheet</h4><p className="text-xs text-slate-300 mb-4 font-medium">Complete 10 practical coding questions to unlock the next video lesson.</p><Button className="w-full bg-white text-black hover:bg-slate-200 font-black rounded-xl h-10 shadow-lg shadow-purple-500/20 text-xs">Start Assignment</Button></div>
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 opacity-50"><div className="h-14 w-20 bg-slate-800 rounded-lg border border-white/10 flex items-center justify-center relative shrink-0"><Lock className="h-4 w-4 text-white/50" /></div><div className="flex-1"><h4 className="font-bold text-white text-xs leading-tight mb-1">1.2 - Solving Linear Equations</h4><p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Video • 8 mins</p></div></div>
              </div>
            </div>
